...
def draw_box(_image, _left, _top, _right, _bottom, _name):
    ...
    cv.rectangle(_image, (_left, _top), (_right, _bottom),
                 BOX_COLOR, BOX_THICKESS)
    cv.putText(_image, _name, (_right+10, _bottom),
...
def get_optimal_font_scale(text, width):
    for scale in range(60, 0, -1):
        textSize = cv.getTextSize(
            text, fontFace=cv.FONT_HERSHEY_DUPLEX, fontScale=scale/10, thickness=1)
...
def get_image_with_boxes(img=None, prediction=None, resize=False, fx=0, fy=0, tolerance=0):
    ...
    for left, top, right, bottom in prediction[0]['boxes']:
        if float(prediction[0]["scores"][h]) > tolerance:
            ...
            array_bbox.append((left, top, right, bottom, name)
                              )                  # Save boxes
...
def show_webcam(MIRROR=False, VIDEO_PATH=0, SHOW_FPS=False, STREAMING=False, RESIZE=False, CONSOLE=False):
    ...
    while frame_exist:
        ...
        if frame_id % FRAME_PRED == 0:
            ...
            FLAG = True
            with torch.no_grad():
                prediction = model([transform_to_tensor(debug_img).to(device)])

            last_bboxes.clear()
            if len(prediction[0]["labels"]) > 0:
                img, last_bboxes = get_image_with_boxes(
                    img=img, prediction=prediction, resize=RESIZE, fx=1/FX, fy=1/FY, tolerance=TOLERANCE)
        ...
        if (FLAG or STREAMING) and SHOW_FPS:
            ...
            TEXT_SIZE = round(get_optimal_font_scale(
                f"FPS: 000000", img.shape[1])/2)
            prev_frame_time = new_frame_time
            fps_history.append(int(fps))
            if not CONSOLE:
                cv.putText(img, f"FPS: { sum(fps_history)/len(fps_history):0.2f}",
                           (7, 70), TEXT_FONT, TEXT_SIZE, TEXT_COLOR, 3, cv.LINE_AA)
